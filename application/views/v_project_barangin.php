...
<body>
    ...
    <title>Inventory Barang</title>
        ...
        <hr>
        <button class="btn btn-success" onclick="fungsitambah_barang()"><i class="glyphicon glyphicon-plus"></i> Tambah Barang Masuk</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('C_project_barangin/ajax_list')?>",
            "type": "POST"
        },
...
function fungsiedit_barang(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('C_project_barangin/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {

            $('[name="id"]').val(data.ID_TRANSAKSI_IN);
            $('[name="ib"]').val(data.ID_BARANG);
            $('[name="il"]').val(data.ID_LOKASI);
            $('[name="ti"]').val(data.TGL_IN);
            $('[name="jb"]').val(data.JML_BARANGIN);
            $('[name="ki"]').val(data.KET_IN);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('C_project_barangin/ajax_add')?>";
    } else {
        url = "<?php echo site_url('C_project_barangin/ajax_update')?>";
    }
...
function fungsihapus_barang(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('C_project_barangin/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Tambah Transaksi Barang Masuk</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <select name="il" class="form-control">
                                    ...
                                    <?php foreach ($isii as $datanya): ?>
                                        <option value="<?php echo $datanya->ID_LOKASI ?>"><?php echo $datanya->NAMA_LOKASI ?></option>
                                    <?php endforeach ?>
                                    </select> 
                                <span class="help-block"></span>
                            </div>
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Keterangan Supllier</label>
                            <div class="col-md-9">
                                <textarea name="ki" placeholder="Masukkan nama supplier beserta ketearnag alamatnya" class="form-control" value=""></textarea>
                                <span class="help-block"></span>
                            </div>
                        </div>
