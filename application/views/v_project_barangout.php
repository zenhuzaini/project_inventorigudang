...
<body>
    ...
    <title>Inventory Barang</title>
        ...
        <hr>
        <button class="btn btn-success" onclick="fungsitambah_barang()"><i class="glyphicon glyphicon-plus"></i> Tambah Data Barang Keluar</button>
        ...
        <table id="table" class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th>No</th>
                    <th>ID Transaksi</th>
                    <th>ID Barang</th><th>Tgl Keluar Barang</th>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('C_project_barangout/ajax_list')?>",
            "type": "POST"
        },
...
function fungsiedit_barang(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('C_project_barangout/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            $('[name="id"]').val(data.ID_TRANSAKSIOUT);
            $('[name="ib"]').val(data.ID_BARANG);
            //$('[name="il"]').val(data.ID_LOKASI);
            $('[name="ti"]').val(data.TGL_OUT);
            $('[name="jb"]').val(data.JML_BARANGOUT);
            $('[name="ki"]').val(data.KET_OUT);
...
function save()
{
    ...
    var url;
    if(save_method == 'add') {
        url = "<?php echo site_url('C_project_barangout/ajax_add')?>";
    } else {
        url = "<?php echo site_url('C_project_barangout/ajax_update')?>";
    }
...
function fungsihapus_barang(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('C_project_barangout/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Tambah Transaksi Barang Keluar</h3>
            </div>
            <div class="modal-body form">
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Tanggal Keluar Barang</label>
                            <div class="col-md-9">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Keterangan Konsumer</label>
                            <div class="col-md-9">
                                <textarea name="ki" placeholder="Masukkan nama Konsumen beserta ketearnag alamatnya" class="form-control" value=""></textarea>
                                <span class="help-block"></span>
                            </div>
                        </div>
